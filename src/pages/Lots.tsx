...
import { nextContracts, type ContractOpt } from '@/lib/pricing';
...
type LotRow = {
  ...
  price_base_contract?: string | null; // jetzt TEXT in der DB
};
...
export default function Lots() {
  ...
  // Für das Drop-down: "other" behandeln wir wie Arabica (KC)
  const contractOptions = useMemo<ContractOpt[]>(
    () => nextContracts(form.species === 'robusta' ? 'robusta' : 'arabica', 8),
    [form.species]
  );
  ...
  function resetForm() {
    const firstContract = nextContracts('arabica', 8)[0]?.value ?? '';
    setForm({
      short_desc: '',
      origin_country: 'Brazil',
      organic: false,
      ...
      price_base_contract: firstContract,
    });
  }
          ...
          <label>Art
            <select className="border rounded px-3 py-2 w-full"
              ...
              onChange={e=>{
                ...
                const first = nextContracts(sp === 'robusta' ? 'robusta' : 'arabica', 8)[0]?.value ?? '';
